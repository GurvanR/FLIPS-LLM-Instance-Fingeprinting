...
class Serial:
    ...
    def serial_test(binary_data:str, verbose=False, pattern_length=16):
        ...
        if verbose:
            logger.debug('Serial Test DEBUG BEGIN:')
            logger.debug("\tLength of input:\t", length_of_binary_data)
            logger.debug('\tValue of Sai:\t\t', sums)
            logger.debug('\tValue of Nabla:\t\t', nabla_01, nabla_02)
            logger.debug('\tP-Value 01:\t\t\t', p_value_01)
            logger.debug('\tP-Value 02:\t\t\t', p_value_02)
            logger.debug('DEBUG END.')

        return ((p_value_01, p_value_01 >= 0.01, nabla_01), (p_value_02, p_value_02 >= 0.01, nabla_02))