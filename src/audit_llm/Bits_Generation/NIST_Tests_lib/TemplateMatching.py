...
class TemplateMatching:
    ...
    def non_overlapping_test(binary_data:str, verbose=False, template_pattern='000000001', block=8):
        ...
        if verbose:
            logger.debug('Non-Overlapping Template Test DEBUG BEGIN:')
            logger.debug("\tLength of input:\t\t", length_of_binary)
            logger.debug('\tValue of Mean (µ):\t\t', mean)
            logger.debug('\tValue of Variance(σ):\t', variance)
            logger.debug('\tValue of W:\t\t\t\t', pattern_counts)
            logger.debug('\tValue of xObs:\t\t\t', xObs)
    ...
    @staticmethod
    def overlapping_patterns(binary_data:str, verbose=False, template_pattern='0000000001', block_size=1032):
        """
        Note that this description is taken from the NIST documentation [1]
        [1] http://csrc.nist.gov/publications/nistpubs/800-22-rev1a/SP800-22rev1a.pdf
        ...
        :param      verbose         True to display the debug messgae, False to turn off debug message
        :param      pattern_size:   the length of the pattern (m)
        :param      block_size:     the length of the block (M)
        :return:    (p_value, bool) A tuple which contain the p_value and result of frequency_test(True or False)
        """
        ...
        if verbose:
            logger.debug('Overlapping Template Test DEBUG BEGIN:')
            logger.debug("\tLength of input:\t\t", length_of_binary_data)
            logger.debug('\tValue of Vs:\t\t\t', pattern_counts)
            logger.debug('\tValue of xObs:\t\t\t', xObs)
            logger.debug('\tP-Value:\t\t\t\t', p_value)
            logger.debug('DEBUG END.')
