...
class FrequencyTest:
    ...
    def monobit_test(binary_data:str, verbose=False):
        ...
        if verbose:
            logger.debug('Frequency Test (Monobit Test) DEBUG BEGIN:')
            logger.debug("\tLength of input:\t", length_of_bit_string)
            logger.debug('\t# of \'0\':\t\t\t', binary_data.count('0'))
            logger.debug('\t# of \'1\':\t\t\t', binary_data.count('1'))
            logger.debug('\tS(n):\t\t\t\t', count)
            logger.debug('\tsObs:\t\t\t\t', sObs)
            logger.debug('\tf:\t\t\t\t\t',fabs(sObs) / sqrt(2))
    ...
    def block_frequency(binary_data:str, block_size=128, verbose=False):
        ...
        if verbose:
            logger.debug('Frequency Test (Block Frequency Test) DEBUG BEGIN:')
            logger.debug("\tLength of input:\t", length_of_bit_string)
            logger.debug("\tSize of Block:\t\t", block_size)
            logger.debug('\tNumber of Blocks:\t', number_of_blocks)
            logger.debug('\tCHI Squared:\t\t', result)
            logger.debug('\t1st:\t\t\t\t', number_of_blocks / 2)
            logger.debug('\t2nd:\t\t\t\t', result / 2)
            logger.debug('\tP-Value:\t\t\t', p_value)
            logger.debug('DEBUG END.')
