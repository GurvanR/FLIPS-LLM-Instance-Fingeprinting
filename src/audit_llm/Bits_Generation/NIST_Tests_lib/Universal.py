...
class Universal:
    ...
    def statistical_test(binary_data: str, verbose=False):
        ...
        if 5 <= pattern_size < 16: # change here so it accepts small binary data
            ...
            if verbose:
                logger.debug("Maurer's Universal Statistical Test DEBUG BEGIN:")
                logger.debug("\tLength of input:\t\t", length_of_binary_data)
                logger.debug("\tLength of each block:\t", pattern_size)
                logger.debug("\tNumber of Blocks:\t\t", init_bits)
                logger.debug("\tValue of phi:\t\t\t", phi)
                logger.debug("\tP-Value:\t\t\t\t", p_value)
                logger.debug("DEBUG END.")

            return (p_value, (p_value >= 0.01), stat)
        else:
            return (-1.0, False, 0)
