...
class RunTest:
    ...
    def run_test(binary_data:str, verbose=False):
        ...
        # TODO: verify the tau formula against NIST SP 800-22 Rev 1a section 2.3.8.
        ...
        if abs(pi - 0.5) >= tau:
            ##logger.debug("The test should not have been run because of a failure to pass test 1, the Frequency (Monobit) test.")
            return (0.0000, False, None) # Paper author: added None to have standardized output
        else:
            # Step 3 - Compute vObs
        ...
        if verbose:
            logger.debug('Run Test DEBUG BEGIN:')
            logger.debug("\tLength of input:\t\t\t\t", length_of_binary_data)
            logger.debug("\tTau (2/sqrt(length of input)):\t", tau)
            logger.debug('\t# of \'1\':\t\t\t\t\t\t', one_count)
            logger.debug('\t# of \'0\':\t\t\t\t\t\t', binary_data.count('0'))
            logger.debug('\tPI (1 count / length of input):\t', pi)
            logger.debug('\tvObs:\t\t\t\t\t\t\t', vObs)
            logger.debug('\tP-Value:\t\t\t\t\t\t', p_value)
            logger.debug('DEBUG END.')

        return (p_value, (p_value > 0.01), vObs)
    ...
    def longest_one_block_test(binary_data:str, verbose=False):
        ...
        if verbose:
            logger.debug('Run Test (Longest Run of Ones in a Block) DEBUG BEGIN:')
            logger.debug("\tLength of input:\t\t\t\t", length_of_binary_data)
            logger.debug("\tSize of each Block:\t\t\t\t", m)
            logger.debug('\tNumber of Block:\t\t\t\t', number_of_blocks)
            logger.debug("\tValue of K:\t\t\t\t\t\t", k)
            logger.debug('\tValue of PIs:\t\t\t\t\t', pi_values)
            logger.debug('\tFrequencies:\t\t\t\t\t', frequencies)
            logger.debug('\txObs:\t\t\t\t\t\t\t', xObs)
            logger.debug('\tP-Value:\t\t\t\t\t\t', p_value)
            logger.debug('DEBUG END.')
