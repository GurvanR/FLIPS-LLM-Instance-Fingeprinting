...
from audit_llm.Bits_Generation.NIST_Tests_lib.BinaryMatrix import BinaryMatrix as bm
...
class Matrix:
    ...
    @staticmethod
    def binary_matrix_rank_test(binary_data: str, verbose=False, rows_in_matrix=32, columns_in_matrix=32):
        """
        ...
        """
        # HARDCODED change for LLM context testing
        rows_in_matrix=8
        columns_in_matrix=8
        ...
        if number_of_block > 0:
            ...
            if verbose:
                logger.debug("Binary Matrix Rank Test DEBUG BEGIN:")
                logger.debug("\tLength of input:\t", length_of_binary_data)
                logger.debug("\tSize of Row:\t\t", rows_in_matrix)
                logger.debug("\tSize of Column:\t\t", columns_in_matrix)
                logger.debug("\tValue of N:\t\t\t", number_of_block)
                logger.debug("\tValue of Pi:\t\t", pi)
