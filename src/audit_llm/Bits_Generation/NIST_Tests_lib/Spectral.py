...
class SpectralTest:
    ...
    def spectral_test(binary_data:str, verbose=False):
        ...
        if verbose:
            logger.debug('Discrete Fourier Transform (Spectral) Test DEBUG BEGIN:')
            logger.debug('\tLength of Binary Data:\t', length_of_binary_data)
            logger.debug('\tValue of T:\t\t\t\t', tau)
            logger.debug('\tValue of n1:\t\t\t', n1)
            logger.debug('\tValue of n0:\t\t\t', n0)
            logger.debug('\tValue of d:\t\t\t\t', d)
            logger.debug('\tP-Value:\t\t\t\t', p_value)
            logger.debug('DEBUG END.')

        return (p_value, (p_value >= 0.01), d)
