...
class ComplexityTest:
    ...
    def linear_complexity_test(binary_data: str, verbose=False, block_size=500):
        ...
        block_size= 20 # hard-coded change to test data
        ...
        if number_of_block > 1:
            ...
            if verbose:
                logger.debug("Linear Complexity Test DEBUG BEGIN:")
                logger.debug("\tLength of input:\t", length_of_binary_data)
                logger.debug("\tLength in bits of a block:\t")
                logger.debug("\tDegree of Freedom:\t\t", degree_of_freedom)
                logger.debug("\tNumber of Blocks:\t", number_of_block)
                logger.debug("\tValue of Vs:\t\t", vg)
